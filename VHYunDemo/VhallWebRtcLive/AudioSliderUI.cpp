...
bool AudioSliderUI::Create() {
   ...
   //m_pVolumeSlider->setObjectName("volumeController");
...
void AudioSliderUI::OnMuteClick() {
 ...
 //  SingletonMainUIIns::Instance().PostCRMessage(MSG_MAINUI_MUTE, &loMute, sizeof(loMute));
	...
	//QJsonObject body;
	//SingletonMainUIIns::Instance().reportLog(L"micmute",eLogRePortK_MicMute, body);
   this->close();
}
...
void AudioSliderUI::OnVolumnChangedSlot(int nValue){
   ...
   globalToolManager->GetDataManager()->WriteLog("%s nValue:%d\n",__FUNCTION__,nValue);
